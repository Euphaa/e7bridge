...
export default class Scheduler
{
    ...
    parsePlayerRequest(words)
    {
        ...
        if (words[1] === "list")
        {
            Main.mineflayerHandler.sendDmTo(words[0], `${[...Scheduler.events.keys()].join(", ")}`);
            return;
        }

        if (words.length < 3)
        {
            Main.mineflayerHandler.sendDmTo(words[0], "not enough args given! usage: /r sub da 1m30s");
            return;
        }
        ...
        {
            Main.mineflayerHandler.sendDmTo(words[0], `${request.event} is not a valid event name! to get a list of events use /r sub list`);
            return;
        }

        else if (request.paddingTime < 1000 || request.paddingTime > 1000*60*60)
        {
            Main.mineflayerHandler.sendDmTo(request.player, "time before notification must be between 1s and 1h. ex: 11m30s");
            return;
        }
        ...
        Main.mineflayerHandler.sendDmTo(request.player, `i will remind you ${Utils.msToTimeNotation(request.paddingTime)} before every ${Scheduler.events.get(request.event).name}.`)
    }
    ...
    unsubscribePlayerFromEvent(player, event)
    {
        ...
        Main.mineflayerHandler.sendDmTo(player, `unsubscribed you from all ${Scheduler.events.get(event).name} reminders!`)
    }
    ...
    notifyPlayerOfEvent(request)
    {
        ...
        Main.mineflayerHandler.sendDmTo(player, `this is a reminder of ${event} in ${Utils.msToTimeNotation(paddingTime)}. to stop notifs of this event, reply with /r sub -u ${request.event}`);
